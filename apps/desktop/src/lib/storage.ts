...
async function createProject(projectData: Partial<ApiProject> & { path?: string }): Promise<{ project: ApiProject }> {
...
async function updateProject(id: string, body: Partial<ApiProject>): Promise<{ project: ApiProject }> {
  const current = (await getProject(id)) || ({ id, user_id: 'local', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), visibility: 'private' } as any)
