...
use crate::scanner::{detect_language, count_lines, count_files};
use rayon::prelude::*;
use serde::Serialize;
use std::path::{Path, PathBuf};
use walkdir::WalkDir;
use git2::{Repository, Oid, DiffOptions, Tree, ObjectType};
use encoding_rs;
...
pub async fn compute_quality_metrics(path: &str, settings: &ScanSettings) -> Result<QualityMetrics, String> {
    let root = Path::new(path);
    if !root.exists() { return Err("Path does not exist".into()); }
    ...
    for entry in WalkDir::new(root).follow_links(settings.follow_symlinks).into_iter() {
        ...
        if entry.path().is_file() {
            if count_files(&entry, settings.clone()) {
                if let Some(s) = entry.path().to_str() { file_paths.push(s.to_string()); }
            }
        }
    }
    ...
    let mut total_lines = 0u32; let mut total_code = 0u32; let mut total_comments = 0u32; let mut total_blank = 0u32;
    for p in &file_paths {
        let path = Path::new(p);
        if let (Some(filename), Ok(content)) = (path.file_name().and_then(|s| s.to_str()), std::fs::read_to_string(path)) {
            let language = detect_language(filename);
            let (t, b, c, code) = count_lines(&content, &language);
            total_lines += t; total_blank += b; total_comments += c; total_code += code;
        }
    }

    let comment_percentage = if total_lines > 0 { (total_comments as f64 / total_lines as f64) * 100.0 } else { 0.0 };
    let code_percentage = if total_lines > 0 { (total_code as f64 / total_lines as f64) * 100.0 } else { 0.0 };

    // Stats
    let mut totals_sorted = file_totals.clone();
    totals_sorted.sort_unstable();
    let avg = if totals_sorted.is_empty() { 0.0 } else { totals_sorted.iter().sum::<u32>() as f64 / totals_sorted.len() as f64 };
    let median = if totals_sorted.is_empty() { 0.0 } else if totals_sorted.len() % 2 == 0 { let m = totals_sorted.len()/2; (totals_sorted[m-1] + totals_sorted[m]) as f64 / 2.0 } else { totals_sorted[totals_sorted.len()/2] as f64 };
    let var = if totals_sorted.is_empty() { 0.0 } else { totals_sorted.iter().map(|&x| { let d = x as f64 - avg; d*d }).sum::<f64>() / totals_sorted.len() as f64 };
    let stddev = var.sqrt();
...
fn path_allowed_by_settings(path: &str, filename: &str, settings: &ScanSettings) -> bool {
    ...
    if settings.excluded_languages.iter().any(|l| l == &lang) { return false; }
    true
}
    ...
    for entry in tree.iter() {
        ...
        match entry.kind() {
            Some(ObjectType::Tree) => {
                if settings.excluded_dirs.iter().any(|d| d == name) { continue; }
            ...
            Some(ObjectType::Blob) => {
                let filename = name;
                if !path_allowed_by_settings(full_path.to_string_lossy().as_ref(), filename, settings) { continue; }
...
fn compute_quality_metrics_for_tree(
    ...
    let avg = if sorted.is_empty() { 0.0 } else { sorted.iter().sum::<u32>() as f64 / sorted.len() as f64 };
    let median = if sorted.is_empty() { 0.0 } else if sorted.len() % 2 == 0 { let m = sorted.len()/2; (sorted[m-1] + sorted[m]) as f64 / 2.0 } else { sorted[sorted.len()/2] as f64 };
    let var = if sorted.is_empty() { 0.0 } else { sorted.iter().map(|&x| { let d = x as f64 - avg; d*d }).sum::<f64>() / sorted.len() as f64 };
    let stddev = var.sqrt();
    ...
    Ok(QualityMetrics {
        ...
        total_comments,
        total_blank,
        comment_percentage: if total_lines > 0 { (total_comments as f64 / total_lines as f64) * 100.0 } else { 0.0 },
        code_percentage: if total_lines > 0 { (total_code as f64 / total_lines as f64) * 100.0 } else { 0.0 },
        avg_file_lines: avg,
        median_file_lines: median,
        stddev_file_lines: stddev,
        dead_code_findings: 0,
...
pub async fn compute_quality_metrics_for_branch(
    ...
    let obj = reference.peel(git2::ObjectType::Commit).map_err(|e| format!("Peel commit failed: {}", e))?;
...
pub async fn compute_branch_quality_deltas(
    ...
    let base_commit = base_ref.peel_to_commit().map_err(|e| format!("Peel base commit failed: {}", e))?;
    let base_tree = base_commit.tree().map_err(|e| format!("Base tree failed: {}", e))?;

    let mut results: Vec<BranchQualityDelta> = Vec::new();

    for b in branches {
        if b == base_branch { continue; }
        let br_ref = match repo.find_reference(&format!("refs/heads/{}", b)) {
            Ok(r) => r,
            Err(_) => { continue; }
        };
        let br_commit = match br_ref.peel_to_commit() { Ok(c) => c, Err(_) => continue };
        let br_tree = match br_commit.tree() { Ok(t) => t, Err(_) => continue };

        let mut diff_opts = DiffOptions::new();
        let diff = repo.diff_tree_to_tree(Some(&base_tree), Some(&br_tree), Some(&mut diff_opts))
        ...
        for d in diff.deltas() {
            ...
            let filename = match (filename, path_str) {
                (Some(f), Some(p)) => {
                    if !path_allowed_by_settings(p, f, settings) { continue };
                    f
                }
                _ => continue,
            };
            ...
            let lang = detect_language(filename);
            let (ot, ob, oc, oo) = match old_text { Some(t) => count_lines(&t, &lang), None => (0, 0, 0, 0) };
            let (nt, nb, nc, no) = match new_text { Some(t) => count_lines(&t, &lang), None => (0, 0, 0, 0) };
...
fn compute_test_coverage(path: &str) -> Option<f64> {
    ...
    if lcov.exists() {
        if let Ok(text) = std::fs::read_to_string(&lcov) {
            let mut total = 0u64; let mut hit = 0u64;
            for line in text.lines() {
                if let Some(rest) = line.strip_prefix("LF:") { if let Ok(v) = rest.trim().parse::<u64>() { total += v; } }
                if let Some(rest) = line.strip_prefix("LH:") { if let Ok(v) = rest.trim().parse::<u64>() { hit += v; } }
            }
            if total > 0 { return Some((hit as f64 / total as f64) * 100.0); }
        }
    }
    ...
    if cov_sum.exists() {
        if let Ok(text) = std::fs::read_to_string(&cov_sum) {
            if let Ok(json) = serde_json::from_str::<serde_json::Value>(&text) {
                if let Some(pct) = json.get("total").and_then(|t| t.get("lines")).and_then(|l| l.get("pct")).and_then(|v| v.as_f64()) {
                    return Some(pct);
                }
            }
        }
...
fn compute_doc_coverage_estimate(path: &str) -> Option<f64> {
    ...
    let mut md = 0u64; let mut files = 0u64;
    for entry in WalkDir::new(path).into_iter().flatten() {
        ...
        if files > 10_000 { break; }
    }
    if files == 0 { None } else { Some((md as f64 / files as f64) * 100.0) }
}
