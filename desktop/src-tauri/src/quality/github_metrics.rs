...
async fn list_prs(
	...
	let filtered = acc
		.into_iter()
		.filter(|pr| match &pr.merged_at {
			Some(m) => {
				if let Ok(ts) = DateTime::parse_from_rfc3339(m) {
...
async fn first_commit_time_for_pr(
	client: &Client,
	slug: &str,
	pr_number: u64,
	token: Option<&str>,
	...
	let url =
		format!("https://api.github.com/repos/{}/pulls/{}/commits?per_page=100", slug, pr_number);
	let mut req = client.get(&url).header("User-Agent", "CodePulse");
	if let Some(t) = token {
		req = req.header("Authorization", format!("Bearer {}", t));
	}
	...
	let min = commits
		.iter()
		.filter_map(|c| DateTime::parse_from_rfc3339(&c.commit.author.date).ok())
		.map(|d| d.with_timezone(&Utc))
		.min();
	Ok(min)
}

pub async fn compute_metrics_for_repo(
	slug: &str,
	weeks: usize,
	token: Option<&str>,
	...
	let since = chrono::DateTime::parse_from_rfc3339(&buckets.first().unwrap().start_iso)
		.unwrap()
		.with_timezone(&Utc);
	...
	for pr in prs {
		let merged_at = match pr.merged_at {
			Some(m) => DateTime::parse_from_rfc3339(&m).ok().map(|d| d.with_timezone(&Utc)),
			None => None,
		};
		let created_at =
			DateTime::parse_from_rfc3339(&pr.created_at).ok().map(|d| d.with_timezone(&Utc));
		if let (Some(m), Some(c)) = (merged_at, created_at) {
			if let Some(idx) = bucket_index_for(&m, &buckets) {
				...
				if let Ok(Some(first_commit)) =
					first_commit_time_for_pr(&client, slug, pr.number, token).await
				{
	...
	let weeks_vec = buckets
		...
		.map(|(i, w)| WeeklyMetrics {
			...
			lead_time_days_avg: if lead_cnt[i] > 0 {
				lead_sum[i] / lead_cnt[i] as f64
			} else {
				0.0
			},
			cycle_time_days_avg: if cycle_cnt[i] > 0 {
				cycle_sum[i] / cycle_cnt[i] as f64
			} else {
				0.0
			},
		})
