...
use git2::{Delta, Diff, DiffDelta, DiffOptions, Oid, Patch};
...
fn extract_file_change(delta: &DiffDelta, diff: &Diff) -> Option<GitFileChange> {
	...
	Some(GitFileChange { path, status: status.to_string(), insertions, deletions })
}
...
pub fn compare_commits(
	...
	let from_oid = Oid::from_str(from_sha).map_err(|_| GitError::CommitNotFound)?;
	let to_oid = Oid::from_str(to_sha).map_err(|_| GitError::CommitNotFound)?;
...
pub fn get_commit_patch(repo_path: &str, commit_sha: &str) -> Result<String, GitError> {
	let repo = super::repo::open_repository(repo_path)?;
