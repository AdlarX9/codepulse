use super::{LanguageStats, ScanResult};
...
pub struct CommitScan {
	pub commit: git_ops::GitCommitInfo,
...
fn should_include_file(path: &str, settings: &ScanSettings) -> bool {
	...
	let lang = super::detect_language(filename);
	if lang == "Unknown" {
		return false;
	}
...
fn count_blob_lines(bytes: &[u8], filename: &str) -> (u32, u32, u32, u32, String) {
	let language = super::detect_language(filename);
	let content = decode_to_string_lossy(bytes);
	let (total, blank, comment, code) = super::count_lines(&content, &language);
	(total, blank, comment, code, language)
}

fn aggregate_files(
	files: &[(String, u32, u32, u32, u32, String)],
) -> (ScanResult, HashMap<String, LanguageStats>) {
	...
	let mean = if totals.is_empty() {
		...
		totals.iter().sum::<u32>() as f64 / totals.len() as f64
	};
...
pub async fn scan_repo_history(
	...
	let commits = git_ops::get_commits(repo_path, None, limit).map_err(|e| e.to_string())?;
